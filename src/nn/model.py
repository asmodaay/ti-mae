...
class MaskedAutoencoder(nn.Module):
    ...
    def forward_loss(self, x, pred, mask,latent):
        ...
        loss = (pred - x.permute(0,2,1)) ** 2
        loss = loss.mean(dim=-1)  # [N, L], mean loss per patch
        inv_mask = (mask -1 ) ** 2
        loss_removed = (loss * mask).sum() / mask.sum()  # mean loss on removed patches
        loss_seen = (loss * inv_mask).sum() / inv_mask.sum()  # mean loss on removed patches
        

        # kl_loss = (- latent.mean() + self.mask_token.mean())**2 + (+latent.std() - self.mask_token.std())**2

        return loss_removed + 0.7*loss_seen
